...
The decision pipeline runs as pure decisions, pinned by 21 tests: from "should this task be
double-run?" through "did the workers agree?" to "what moves on the rail, and to whom." None of
*that* touches the chain -- on purpose, so every rule is provable before any money is at stake. The
chain is reached only past the last decision, through one injected `SlashSubmitter` (see
[The bond, on chain](#the-bond-on-chain)); that wire is proven live once on testnet-10 (2026-09-21).
...
**The wiring** (self-audit, 2026-09-16 -- the author's own pass, not a third party's; broadcaster added 2026-09-20): most of the above is proven covenant *mechanics* exercised only by hand-run `live-*` scripts. Two wires into the product now exist. [src/execute.ts](src/execute.ts) turns a `pay-and-slash` *decision* into an executable `SlashSpend` -- the worker's real quorum-bond redeem, the buyer's payout, and the exact `guiltyDigest` the compiled covenant recomputes on chain (byte-tied to the same digest the live bond proved). [src/broadcast.ts](src/broadcast.ts) is the wire's end: `broadcastSettlementSlashes(chain, parties, settlement, fee)` takes a settlement and, on `pay-and-slash`, lands one real covenant spend per caught worker -- fee = bond − payout, the referee's datasig over the guilt digest, the witness `[sig, tag, redeem]` -- so `settle()` **ends in an on-chain slash, not a recipe**. The chain call is an injected `SlashSubmitter`, so the whole path is unit-tested offline (8 tests) with quorum still SDK-free. And because a txid is not finality on a BlockDAG, [src/confirmations.ts](src/confirmations.ts) + [src/settlementledger.ts](src/settlementledger.ts) (added 2026-09-21, after vprogs PR #146) hold each broadcast slash tagged with the block it landed in: a slash counts only once it is a confirmation floor behind the tip, and a reorg hands back the orphaned slashes so their deeds get un-dinged off chain -- keeping the fabric's record in step with the chain. `runSettlementSlashes(chain, parties, settlement, fee, ledger)` is the whole path in one call. **Proven live on testnet-10 (2026-09-21):** the product code -- not a hand-run script -- posted a bond, broadcast the slash (tx [`81c3008f1fe5d79508105ada9b0760f4de52651de8b38a5039f549aeffaf172d`](https://explorer-tn10.kaspa.org/txs/81c3008f1fe5d79508105ada9b0760f4de52651de8b38a5039f549aeffaf172d): 1 KAS in from the P2SH bond, 0.99 KAS out to the buyer, 0.01 KAS fee, accepted at blue score 565268486), consensus paid the buyer, and the ledger tagged it at its real block DAA so a reorg would revert it (driver: `kaspa-depin/scripts/live-slash.ts`). The wire that self-audit found missing is closed end to end.

**The proofs, kept.** Every transaction this repository links to an explorer is also archived as
[docs/proofs/](docs/proofs/)`<txid>.json` -- the transaction as `api-tn10.kaspa.org` served it,
with when it was fetched. The public testnet-10 index serves only about the last six days
(measured 2026-09-22), after which a link alone is the author's word. `npm run harvest` archives
anything newly cited; `npm test` fails on any linked id without its file, and on any id that was
cut short.
