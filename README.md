...
**The wiring, started** (audit, 2026-09-16): most of the above is proven covenant *mechanics* exercised only by hand-run `live-*` scripts. The first wire into the product now exists: [src/execute.ts](src/execute.ts) turns a `pay-and-slash` *decision* into an executable `SlashSpend` -- the worker's real quorum-bond redeem, the buyer's payout, and the exact `guiltyDigest` the compiled covenant recomputes on chain (byte-tied to the same digest the live bond proved). `slashesFor(parties, settlement.slashed, fee)` is the top of it. **Still to close:** a broadcaster (the WASM live layer that signs and submits the `SlashSpend`), and having `plan.ts`'s `slash-bond` actions invoke it automatically -- so `settle()` ends in an on-chain slash, not a recipe. Wiring, not more covenants.
